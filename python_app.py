...
# -----------------------------------------------------------------------------
st.set_page_config(
    page_title="Puffy Lux PDP Redesign — A/B Experiment Intelligence (Pandas)",
    page_icon="🐍",
    layout="wide",
...
def generate_pdf_report(fig_funnel, fig_cond, fig_scroll, fig_water,
  ...
  c.setFillColor(colors.HexColor("#94A3B8"))
  c.drawString(margin, height - 1.4 * inch, "A/B Experiment Intelligence Report (Pandas)")
  ...
  gap2 = 0.3 * inch
  chart_h = (height - top_offset - bottom_offset - gap2) / 2
  ...
  c.drawImage(img2, margin, height - top_offset - 2 * chart_h - gap2,
              width=chart_w, height=chart_h, preserveAspectRatio=True, mask="auto")
  ...
  c.drawImage(img4, margin, height - top_offset - 2 * chart_h - gap2,
              width=chart_w, height=chart_h, preserveAspectRatio=True, mask="auto")
...
with pdf_slot.container():
  if st.button("📄 Generate PDF Report", key="pdf_gen_py"):
    with st.spinner("Rendering charts to PDF..."):
      pdf_buffer = generate_pdf_report(
          fig_funnel, fig_cond, fig_scroll, fig_water, cr_a, cr_b, rpu_a, rpu_b
      )
    st.session_state["pdf_buffer_py"] = pdf_buffer.getvalue()

  if "pdf_buffer_py" in st.session_state:
    st.download_button(
        label="⬇️ Download PDF",
        data=st.session_state["pdf_buffer_py"],
        file_name="puffy_pdp_ab_report_pandas.pdf",
